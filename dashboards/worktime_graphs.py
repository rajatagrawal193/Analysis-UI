...
class WorktimeGraphs:
    ...
    DAILY_WORKTIME_EVENTS_GRAPH_ID = "daily_worktime_events_graph"
    ...
    def __init__(self, start_date, end_date):
        ...
        self.content = html.Div(id=self.WORKTIME_GRAPHS_CONTENT_ID, children=[
            dcc.Graph(id=self.WORKTIME_EVENTS_GRAPH_ID, animate=True, figure=self.get_worktime_events_graph()),
            dcc.Graph(id=self.DAILY_WORKTIME_EVENTS_GRAPH_ID, animate=True, figure=self.get_daily_worktime_graph()),
            dcc.Graph(id=self.WEEKLY_WORKTIME_EVENTS_GRAPH_ID, animate=True, figure=self.get_weekly_worktime_graph()),
            dcc.Graph(id=self.MONTHLY_WORKTIME_EVENTS_GRAPH_ID, animate=True, figure=self.get_monthly_worktime_graph()),
    ...
    def get_worktime_events_graph(self):
        graph = Graph(f"Worktime Events| Total Events {len(self.worktime_events)} ", "Date", "Time")
        for event in self.worktime_events:
    ...
    def get_weekly_worktime_graph(self):
        # return go.Figure()
        return generate_trend("Worktime", self.worktime_events, self.start_date, self.end_date, "Weekly", duration=True)

    def get_monthly_worktime_graph(self):
        # return go.Figure()
        return generate_trend("Worktime", self.worktime_events, self.start_date, self.end_date, "Monthly", duration=True)

    def get_daily_worktime_graph(self):
        return generate_trend("Worktime", self.worktime_events, self.start_date, self.end_date, "Daily", duration=True)
